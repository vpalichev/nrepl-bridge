#!/usr/bin/env bb
;; test-phase4.bb -- Run Phase 4 acceptance tests: edge cases + adversarial stress
;;
;; Spec tests T4.1-T4.5 plus aggressive extras T4.6-T4.18.
;;
;; Prerequisites:
;;   1. npx shadow-cljs watch app  (nREPL on 17888)
;;   2. One browser tab on http://localhost:8280
;;
;; Usage: bb nrepl-bridge/test-phase4.bb [--backend-port 17888]
...
(println "=== Phase 4 Acceptance Tests: Edge Cases + Adversarial ===")
...
(let [server (proc/process
  ...
  (try
    ...
    (letfn [(eval! [form & opts]
      ...
      ;; T4.1 -- Multi-form evaluation
      (println "--- T4.1: Multi-form ---")
      (let [resp (eval! "(def x 10)\n(def y 20)\n(+ x y)")
            text (get-text resp)]
        (record! "T4.1" "Multi-form (def x 10)(def y 20)(+ x y) = 30"
                 (and text (ci? text "30"))
                 (str "Got: " (pr-str text))))

      ;; T4.2 -- Large output
      (println "--- T4.2: Large output ---")
      (let [resp (eval! "(vec (range 10000))" :timeout_ms 15000)
        ...
        (record! "T4.2" "Large output (range 10000) handled"
      ...
      ;; T4.3 -- Exception handling
      (println "--- T4.3: Exception (/ 1 0) ---")
      (let [resp (eval! "(/ 1 0)")
            text (get-text resp)
            err? (is-error? resp)]
        ...
        (record! "T4.3" "Division by zero returns error"
                 (and text (or (ci? text "Arithmetic")
                               (ci? text "Divide by zero")))
                 (str "isError=" err? " text=" (pr-str text))))

      ;; T4.4 -- Namespace persistence
      (println "--- T4.4: Namespace persistence ---")
      (eval! "(def ^:dynamic *phase4-marker* 42)")
      (let [resp (eval! "*phase4-marker*")
            text (get-text resp)]
        (record! "T4.4" "Var defined in one call, read in next"
                 (and text (ci? text "42"))
                 (str "Got: " (pr-str text))))

      ;; T4.5 -- Audit trail completeness
      (println "--- T4.5: Audit trail ---")
      (let [db-path ".workbench/db/toolchain.db"
        ...
        (record! "T4.5" "Audit trail: no orphaned pending rows"
                 (and cnt (pos? cnt) (zero? pending-cnt))
                 (str "Total: " cnt " Pending: " pending-cnt)))

      ;; ===== ADVERSARIAL TESTS =====

      ;; T4.6 -- Deeply nested data
      (println "--- T4.6: Deeply nested data ---")
      (let [resp (eval! "(reduce (fn [acc _] {:nested acc}) :leaf (range 50))")
            text (get-text resp)]
        (record! "T4.6" "50-level nested map round-trips"
                 (and text (ci? text "nested") (ci? text "leaf"))
                 (str "Got " (count (or text "")) " chars")))

      ;; T4.7 -- All shell-dangerous chars in one form
      (println "--- T4.7: All shell-dangerous chars combined ---")
      (let [resp (eval! (str "(str \"/api/health\" \" $HOME \" \"`whoami`\""
                             " \" !event! \" \"$(pwd)\" \" \\\\n\\\\t\\\\r\""
                             " \" 'single' \" \"\\\"double\\\"\" \" C:\\\\Users\")"))
            text (get-text resp)
            val  (unquote-str text)]
        (record! "T4.7" "All shell traps in one string"
                 (and val
                      (ci? val "/api/health")
                      (ci? val "$HOME")
                      (ci? val "`whoami`")
      ...
      ;; T4.8 -- Long form (stress test form length)
      (println "--- T4.8: Long form (4KB) ---")
      (let [;; Generate a form with many additions: (+ 1 1 1 ... 1) with 1000 ones
            ...
            text (get-text resp)]
        (record! "T4.8" "4KB form with 1000 addends = 1000"
                 (and text (ci? text "1000"))
                 (str "Got: " (pr-str text))))

      ;; T4.9 -- Rapid-fire sequential evals
      (println "--- T4.9: Rapid-fire 20 evals ---")
      (let [start (System/currentTimeMillis)
            ...
            all-ok (every? (fn [{:keys [i text]}]
                           ...
                           results-vec)]
        (record! "T4.9" (str "20 rapid evals in " elapsed "ms, all correct")
                 all-ok
                 (str "elapsed=" elapsed "ms failed="
                      (pr-str (filterv #(not (ci? (or (:text %) "") (str (inc (:i %)))))
                                       results-vec)))))

      ;; T4.10 -- Form returning nil
      (println "--- T4.10: nil return ---")
      (let [resp (eval! "(when false :never)")
            text (get-text resp)]
        (record! "T4.10" "nil return handled gracefully"
                 (and text (ci? text "nil"))
                 (str "Got: " (pr-str text))))

      ;; T4.11 -- stdout + stderr + return value combined
      (println "--- T4.11: stdout + stderr + return ---")
      (let [resp (eval! (str "(do (println \"stdout-line\")"
                             " (.println System/err \"stderr-line\")"
                             " :final-value)"))
            text (get-text resp)]
        (record! "T4.11" "stdout + stderr + return all captured"
                 (and text
                      (ci? text "stdout-line")
                      (ci? text ":final-value"))
                 (str "Got: " (pr-str (when text (subs text 0 (min 200 (count text))))))))

      ;; T4.12 -- StackOverflow handling
      (println "--- T4.12: StackOverflow ---")
      (let [resp (eval! "((fn f [] (f)))" :timeout_ms 10000)
            text (get-text resp)
            err? (is-error? resp)]
        ;; Exception may be signaled via isError or stderr text.
        (record! "T4.12" "StackOverflow returns error, no crash"
                 (and text (or (ci? text "StackOverflow")
                               (ci? text "stack")))
                 (str "isError=" err? " text="
                      (pr-str (when text (subs text 0 (min 150 (count text))))))))

      ;; T4.13 -- Windows backslash paths
      (println "--- T4.13: Windows backslash paths ---")
      (let [resp (eval! "(str \"C:\\\\Users\\\\v.palichev\\\\project\")")
            text (get-text resp)
            val  (unquote-str text)]
        (record! "T4.13" "Windows paths with backslashes preserved"
                 (and val (ci? val "C:\\Users\\v.palichev\\project"))
                 (str "Got: " (pr-str val))))

      ;; T4.14 -- Unicode edge cases: ZWJ, RTL, combining chars
      (println "--- T4.14: Unicode edge cases ---")
      (let [;; Family emoji (ZWJ sequence), Arabic RTL, combining diacritical
            resp (eval! (str "(str \"\u0410\u0301\" \"|\" \"\u0627\u0644\u0639\u0631\u0628\u064a\u0629\""
                             " \"|\" \"\u200b\" \"|\" \"\u2066test\u2069\")"))
            text (get-text resp)]
        (record! "T4.14" "ZWJ, RTL, combining chars survive"
                 (and text
                      (ci? text "\u0410")   ;; Cyrillic A
                      (ci? text "\u0627"))   ;; Arabic alef
                 (str "Got hex: " (to-hex (unquote-str text)))))

      ;; T4.15 -- SQL injection attempt in form
      (println "--- T4.15: SQL injection in form text ---")
      (let [resp (eval! "(str \"'; DROP TABLE evals; --\")")
            ...
            cnt  (:cnt (first rows))]
        (record! "T4.15" "SQL injection attempt: form evals, DB intact"
                 (and val
                      (ci? val "DROP TABLE")
                      cnt (pos? cnt))
                 (str "val=" (pr-str val) " db-rows=" cnt)))

      ;; T4.16 -- Frontend edge case via cljs-eval
      (println "--- T4.16: Frontend math via target:frontend ---")
      ;; Requires a browser tab on localhost:8280. Fail hard if missing.
      (let [check (eval! "(count (shadow.cljs.devtools.api/repl-runtimes :app))")
        ...
        (when-not has-browser
          (record! "T4.16" "Frontend: sum 1..100 = 5050" false
                   "No browser connected. Open http://localhost:8280 and re-run.")
          (.close out)
          (proc/destroy server)
          (println "\nFATAL: T4.16 requires a browser on localhost:8280.")
          (System/exit 1))
        (let [resp (eval! "(reduce + (range 1 101))" :target "frontend" :timeout_ms 15000)
              text (get-text resp)]
          (record! "T4.16" "Frontend: sum 1..100 = 5050"
                   (and text (ci? text "5050"))
                   (str "Got: " (pr-str text)))))

      ;; T4.17 -- Empty string and whitespace-only forms
      (println "--- T4.17: Whitespace / empty edge cases ---")
      (let [resp-ws (eval! "   \n\t  ")
            text-ws (get-text resp-ws)
            err-ws? (is-error? resp-ws)]
        (record! "T4.17" "Whitespace-only form handled gracefully"
                 ;; Should error or return nil, not crash
                 (boolean (or err-ws? text-ws))
                 (str "isError=" err-ws? " text=" (pr-str text-ws))))

      ;; T4.18 -- Multiple string escapes stacked
      (println "--- T4.18: Stacked escape sequences ---")
      (let [resp (eval! "(pr-str {:a \"line1\\nline2\" :b \"tab\\there\" :c \"quote\\\"inside\\\"\"})")
            text (get-text resp)]
        (record! "T4.18" "Stacked \\n \\t \\\" in pr-str round-trip"
                 (and text
                      (ci? text "line1")
                      (ci? text "line2")
                      (ci? text "tab")
